...
function Fwz_eval34(&$obj,$type,$calibrate=1){
	...
	if($range!=0){
		//$range/=1.15;
		//$obj['rangeMod']=(1-pow(.996,pow($range,2)))/(1-pow(.996,pow(6,2)));
		//$fac=(1+.22/(1-.22))**.5; //dependent on the hp of units
	...
	$sno=$data['sensor']['k']['ID'][ $obj['Sensor key'] ];
	$obj['Prevalue']+=$data['sensor'][$sno]['Prevalue'];
...
function Fwz_eval(&$obj,$type,$calibrate=1){
	global $sys;
	global $data;
	//echo 'TYPE--:' .$type;
	if($type=='templates'){
	...
	if($type=='structures'){
		...
		$bodyval=$fun['structval'];
		$weapval=1;
		
	}
	if($obj['splash damage']>0){
		$obj['sdmg']=Fwz_armor($obj['radiusDamage'])*(pow($obj['radius']/128,1.41)-1/4)*$obj['radiusLife']/100/2;
	}
	$obj['kdmg']=Fwz_armor($obj['damage'])*($obj['shortHit']+$obj['longHit'])/100/2;
	if($obj['incenTime']>0){
		$obj['fdmg']=Fwz_armor($obj['periodicalDamage']+15*5)*pow(($obj['periodicalDamageRadius'])/128,1.41)*$obj['periodicalDamageTime']/20/3;
	}
	if($obj['weaponSubClass']=='EMP'){
		$obj['edmg']=$calibrate*$expect*2;
	}
	...
	if($range!=0){
		$obj['rangeMod']=(1-pow(.996,pow($range,2)))/(1-pow(.996,pow(5,2)));
	}
	else{
		$obj['rangeMod']=1;
	}
	...
	$obj['miscMod']*=$obj['penetrate']*1.5+1;
	$obj['miscMod2']=-($obj['fireOnMove']=="NO")*.4+1;
	#$obj['HPmod']=($obj['hitpoints']*$bodyval+$boost['hitpoints'])/($basehp=250)*4;
	$hp2=$obj['hitpoints'];
	if($type=='body'){
	...
	$slotmult=1;
	//$weapval*=450/2800; //damage equilibrium
	$obj['propHP']=$hp2/($hp2+$boost['HP']);
	$obj['HPmod']=($hp2+$boost['HP'])*$bodyval;
	$meddmg=30*pow($expect,2); //both upgrade and turret
	$medHp=($sys['wz']['mean']['body']['HP']*311/135+$sys['wz']['mean']['weapons']['HP'])*pow($expect,2);
	$obj['armDmg']=$meddmg;
	$obj['armAprox']=min($meddmg-.1,($obj['Armour']*4+$obj['AFRK']*3+$obj['AFRH'])/4);
	$obj['armMod']=$meddmg/($meddmg-$obj['armAprox']);
	//$obj['evalStr'].='m:'. $meddmg .'/'. ($obj['AFRK']*2+$obj['AFRH'])/3;
	if($obj['weight']!=''){
		$obj['weightMod']=pow(7000/($obj2['weight']+$boost['weight']),.3);
	...
	$obj['evalStr'].='+'. floor($obj['edmg']);
	$obj['evalStr'].=')*'. floor(1000*$obj['rof'])/1000;
	$obj['evalStr'].='*'. floor($obj['rangeMod']*10)/10;
	$obj['evalStr'].='*<u>'. floor(10*$obj['miscMod'])/10 .'</u>';
	$obj['evalStr'].='*'. floor(10*$weapval)/10;
	//$obj['evalStr'].='*?'. floor(100*$propDmg)/100;
	//$obj['evalStr'].='--'. floor(100*$delta)/100;
	...
	$obj['evalStr'].='</i>*'. floor($hp2*10)/10;
	//$obj['evalStr'].='</i>*'. floor($bodyval*10)/10;
	$obj['evalStr'].='*'. floor($bodyval*100)/100;
	//$obj['evalStr'].='*('. floor($bodyval*100)/100 .'-'. $hp2 .' '. $obj['hitpoints'] .' '. $boost['hitpoints'] .')';
	$obj['evalStr'].='*'. floor($obj['armMod']*100)/100;
	$obj['evalStr'].='('. floor($expect*100)/100 .')';
	$obj['evalStr'].='=><i>'. floor(10*$obj['Prevalue2'])/10 .'</i>';
