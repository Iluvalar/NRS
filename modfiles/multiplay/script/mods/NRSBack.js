...
function powerStuff(){
	...
	var factories=0;
	var droids =0;
	var basebank=3+getMultiTechLevel()
	...
	for (var playnum = 0; playnum < maxPlayers; playnum++){
		...
		honorPow+=countStruct("A0ComDroidControl", playnum)*500*1;
	...
	for (var playnum = 0; playnum < maxPlayers; playnum++){
		...
		basePow=(basebank-banks[playnum])*basepower/5;
	...
	if (playerData[player].difficulty == INSANE)*/
		...
		*/
		...
		var fractprop=2**((totpropPow)/(basepower*maxPlayers/2));
		//if(totpropPow!=0){
		...
		if(playerData[playnum].difficulty>0){
			...
			factordif=3.5;
			if(playerData[playnum].difficulty==1){ factordif=1.5+aiFact; }
			if(playerData[playnum].difficulty==2){ factordif=1.95+aiFact; }
			if(playerData[playnum].difficulty==3){ factordif=2.20+aiFact; }
		...
		honor[playnum]+=(honorPow+( (ndiff-6)/maxPlayers)*500 )*5*1000;
		powerGen[playnum]=income/10; //
		//powerLast[playnum]+=income;
		var powMult=powertypefact/fractprop*foundsFact*(1-equity);
		if(powMult<0){ powMult=0; }
		setPowerModifier(powMult, playnum);
	...
	if(tick%3==0){
		...
		conText+=" propaganda:"+  Math.ceil(totpropPow)  +"("+  Math.ceil(fractprop*100)/100 +") founds"+ Math.ceil((1-founds)*100) +" equ:"+ conDeb;
	}if(tick%3==1){
		if(tick==1){
